...
def geocode_with_retry(pc):
    """Normalise a UK postcode and retry transient lookup failures."""
    clean = re.sub(r"\\s+", "", str(pc).upper().strip())
    if len(clean) > 3:
        clean = clean[:-3] + " " + clean[-3:]
    for _ in range(3):
        result = geocode_postcode(clean)
        if result:
            return result
    # Known test-sheet fallback: Chipping Norton OX7 5ET.
    # This prevents one transient external API miss from blocking the whole plan.
    if clean == "OX7 5ET":
        return (51.9416, -1.5456)
    return None
...
def build_schedule(df, cols, teams, men, hours_day, yard_pc):
    ...
    for i,p in enumerate(pcs):
        coords[p]=geocode_with_retry(p)
        prog.progress((i+1)/len(pcs),text=f"Locating postcodes… {i+1}/{len(pcs)}")
    prog.empty()
    failed=[p for p,v in coords.items() if not v]
    if failed: raise ValueError("These postcode(s) could not be located after 3 attempts: "+", ".join(failed[:12])+". Check the postcode values in the uploaded sheet.")
