...
def ors_matrix(points):
    ...
    Large monthly lists are split into smaller Matrix requests so the public
    ORS endpoint is not sent too many locations in one request.
    """
    key = ors_key()
    if not key:
    ...
    # Keep the union of source + destination locations comfortably below
    # the hosted Matrix location limit. 12 + 12 = max 24 locations/request.
    block=12
